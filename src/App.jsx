...
const PSF_PARAMS = {pi_c:49.6,pi_p:124.0,w_c:248.0,kappa:0.87};
const PSF_DATA = [{pi:4.96,w:170.33},{pi:11.21,w:190.55},{pi:17.46,w:207.74},{pi:23.71,w:221.87},{pi:29.96,w:232.96},{pi:36.21,w:241.01},{pi:42.46,w:246.01},{pi:48.71,w:247.97},{pi:54.96,w:246.88},{pi:61.21,w:242.75},{pi:67.46,w:235.57},{pi:73.71,w:225.34},{pi:79.96,w:212.07},{pi:86.2,w:195.79},{pi:92.45,w:176.43},{pi:98.7,w:154.03},{pi:104.95,w:128.58},{pi:111.2,w:100.09},{pi:117.45,w:68.56},{pi:123.7,w:33.98},{pi:129.95,w:-3.65},{pi:136.2,w:-44.32},{pi:142.45,w:-88.04},{pi:148.7,w:-134.8},{pi:154.95,w:-184.61},{pi:161.2,w:-237.46}];

const MC_PARAMS = {n_draws:100000,mean:12.4,ci_lo:8.68,ci_hi:22.32,pct_hw:97.4,channels:[{name:"Primary welfare channel",dist:"LogNormal",lo:3.72,hi:8.68},{name:"Secondary externality",dist:"Triangular",lo:1.24,hi:4.96},{name:"Governance / institutional",dist:"Uniform",lo:0.62,hi:2.48}]};
const MC_DATA = [{bin:"0.7",count:223},{bin:"2.0",count:532},{bin:"3.4",count:1142},{bin:"4.7",count:2206},{bin:"6.0",count:3833},{bin:"7.4",count:5993},{bin:"8.7",count:8430},{bin:"10.1",count:10670},{bin:"11.4",count:12152},{bin:"12.8",count:12452},{bin:"14.1",count:11480},{bin:"15.4",count:9523},{bin:"16.8",count:7108},{bin:"18.1",count:4773},{bin:"19.5",count:2884},{bin:"20.8",count:1568},{bin:"22.2",count:767},{bin:"23.5",count:337},{bin:"24.9",count:133},{bin:"26.2",count:47}];

const THRESHOLDS = [{domain:"Gene Drive Deployment — regulatory trigger",year:2030,status:"Institutional response threshold approaching",confidence:"Medium",crossed:false},{domain:"Gene Drive Deployment — welfare crossover",year:2038,status:"Projected welfare-cost crossover point",confidence:"High",crossed:false},{domain:"Gene Drive Deployment — systemic failure",year:2048,status:"System-level failure if unaddressed",confidence:"Medium-Low",crossed:false},{domain:"Gene Drive Deployment — irreversibility",year:2061,status:"Irreversible welfare destruction threshold",confidence:"Low",crossed:false}];

const AXIOMS = {type:"institutional",items:[{id:"I1",name:"Regulatory Capture",description:"Regulatory bodies governing Gene Drive Deployment have been structurally captured by incumbent firms, preventing welfare-preserving intervention."},{id:"I2",name:"Information Asymmetry",description:"Welfare costs of Gene Drive Deployment are distributed across populations that lack the information to price them, while benefits are concentrated."},{id:"I3",name:"Temporal Displacement",description:"Welfare destruction from Gene Drive Deployment is temporally displaced — costs materialize years or decades after extraction, defeating feedback mechanisms."}]};

const METHODS_DATA = {
  welfare_function: "W(t) = W_0 - \u03A3_i \u03B2_W(i) \u00B7 \u03A0_i(t) + \u03B7 \u00B7 R(t), where R(t) represents remediation effort. For Gene Drive Deployment, W_0 is calibrated to pre-extraction baseline welfare. The aggregate \u03B2_W = 12.4 implies $12.4 of welfare destruction per $1 of private payoff.",
  cooperative_baseline: "The cooperative extraction level \u03A0_C = $49.6B maximizes joint surplus W + \u03A0. Current extraction \u03A0_P = $124.0B exceeds this by $74.4B, representing the over-extraction gap. Shadow price \u03BC* = 0.0806 (8.1%) defines the efficient welfare price.",
  falsification: ["If \u03B2_W < 1.0 under any reasonable social cost methodology, the PST classification fails for Gene Drive Deployment.","If welfare costs are fully internalized through existing market mechanisms (Coase conditions hold), the institutional failure claim is falsified.","If Monte Carlo robustness drops below 80% for \u03B2_W > 1.0, the point estimate is unreliable.","If cooperative baseline extraction \u03A0_C exceeds current \u03A0_P, over-extraction claim is reversed."],
  key_sources: ["Postnieks, E. (2026). System Asset Pricing Model: Gene Drive Deployment. SAPM Working Paper.","Rennert, K. et al. (2022). Comprehensive evidence implies a higher social cost of CO\u2082. Nature 610.","Arrow, K. (1951). Social Choice and Individual Values. Impossibility theorem foundations.","Stiglitz, J. (2019). People, Power, and Profits. Institutional failure mechanisms.","Nordhaus, W. (2017). Revisiting the social cost of carbon. PNAS 114(7)."]
};
